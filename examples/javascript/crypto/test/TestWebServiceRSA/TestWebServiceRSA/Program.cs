...
namespace TestWebServiceRSA
{
    ...
    class RSACryptoStream
    {
        ...
        {
            get { return dwKeySizeToEncryptedDataLength(this.rsa.KeySize); }
        }
        ...
        public void WriteTo(byte[] bytes, Stream stream)
        {
            ...
            var chuncksize = MaxDataFromdwKeySize(this.rsa.KeySize);

            var data = new byte[chuncksize];
            var datalength = m.Read(data, 0, chuncksize);
            ...
            while (datalength > 0)
            {
                ...
                var unclear = rsa.Encrypt(data, fOAEP: false);

                stream.Write(unclear, 0, unclear.Length);

                datalength = m.Read(data, 0, chuncksize);
            }
        }
        ...
        public byte[] ReadAllBytes(Stream stream)
        {
            ...
            while (datalength > 0)
            {
                ...
                var clear = rsa.Decrypt(data, fOAEP: false);
