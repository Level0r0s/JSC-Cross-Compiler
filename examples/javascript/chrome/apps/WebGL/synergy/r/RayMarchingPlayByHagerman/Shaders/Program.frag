...
{
    return length(p)-abs(sin(p.z+iGlobalTime));
}
...
{
    //vec2 res = vec2( sdSphere(    pos-vec3( 0.0,0.0, 0.0), 0.25 ), 46.9 );
    vec2 res = opU( vec2( sdPlane(     pos), 1.0 ),
	                vec2( sdSphere(    pos-vec3( 0.0,0.0, 0.0), 0.25 ), 46.9 ) );
...
{ 
    ...
    if( m>-0.5 )
    {
        ...
        // material        
		col = 0.45 + 0.3*sin( vec3(1.0,0.5,0.0)*(m-1.0)+sin(10.0*iGlobalTime) );
