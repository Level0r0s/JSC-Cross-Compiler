...
vec3 smallTrianglesColor(vec3 pos)
{
    ...
    type += 0.2 * sin(iGlobalTime*5.0*type);
...
{
	float d = length(pos.xz - vec2(0.0, 2.0) + 0.5*cos(2.0*pos.xz + vec2(3.0, 1.0) * iGlobalTime)) +  pos.y + 0.2 * cos(pos.y - iGlobalTime);
    float border = 12.0 * mod(iGlobalTime * 0.2, 1.0);
