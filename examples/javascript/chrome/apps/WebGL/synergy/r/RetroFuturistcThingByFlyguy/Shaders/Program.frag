...
vec3 Shade(MarchResult hit, vec3 direction, vec3 camera)
{
    ...
    //Distance fog
    color *= 1.0 - (0.5*smoothstep(0.0, MAX_DIST*0.9, hit.dist));
    ...
    {
        //Sky gradient
    	color = mix(SKY_COLOR_1, SKY_COLOR_2, -hit.position.z/216.0);
        
        //Sun
        vec3 sunDir = normalize(SUN_DIRECTION);
        
        float sun = smoothstep(0.9950, 0.9952, dot(direction, sunDir));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	// if we change code here.
	// can jsc assetsbuild detect it and let the running apps know via UDP that a new version is available?

	// #define FOCAL_LENGTH 0.8
    //vec3 dir = normalize(vec3(uv - res / 2.0, FOCAL_LENGTH)) * rotate;
    //vec3 dir = normalize(vec3(uv - res / 2.0, 1.0)) * rotate;
    vec3 dir = normalize(vec3(uv - res / 2.0, 2.0)) * rotate;
