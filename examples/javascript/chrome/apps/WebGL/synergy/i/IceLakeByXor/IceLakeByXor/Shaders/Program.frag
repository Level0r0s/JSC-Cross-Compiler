...
{
    vec2 dir = ((iMouse.xy/iResolution.xy) * vec2(1.0,0.5) + vec2(0.0,0.5)) * 6.2831;
    vec3 pos = vec3(noise(vec2(0.02,0.01)*iGlobalTime)*20.0,0.5,iGlobalTime*0.2);
	camPos =  pos;
...
vec3 doBackground( in vec3 dir)
{
    ...
    vec2 p = dir.xz/dir.y+vec2(iGlobalTime*0.05,0.0);
