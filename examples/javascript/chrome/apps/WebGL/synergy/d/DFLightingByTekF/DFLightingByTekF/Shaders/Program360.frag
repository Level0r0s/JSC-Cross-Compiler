...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	rotCam = false;
	Camera camKeys3 = SetCam( vec3(30,2.5-SSradius,5), vec3(30,2.5-SSradius,5) + uCameraTargetOffset, vec3(0,1,0), .75 );
	Camera cam  = camKeys3;
...
//rotCam = false; cam = camKeys[3];
	...
	//vec3 ray = normalize(vec3( fragCoord.xy-iResolution.xy*.5, 1.0));


	//vec2 uv = (2.0*fragCoord.xy -iResolution.xy)/ iResolution.y;
	vec2 uv = fragCoord.xy-iResolution.xy
	*.5
	// makes cam move onthefloor?
	//*.55
	;
	...
	//vec3 ray = normalize(vec3( uv, iResolution.x * 0.75 ));
	vec3 ray = normalize(vec3( uv, iResolution.x * 1.0 ));
