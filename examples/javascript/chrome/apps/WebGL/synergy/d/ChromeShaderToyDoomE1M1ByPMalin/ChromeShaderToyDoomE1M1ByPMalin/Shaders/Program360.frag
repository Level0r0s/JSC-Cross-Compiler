...
// https://sites.google.com/a/jsc-solutions.net/work/knowledge-base/15-dualvr/20151027/x360doome1m1
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	// whats this magic number?

	//vWindowCoord /= 0.55;
	//vWindowCoord /= 0.54;
	// vWindowCoord /= 0.56;
	//vWindowCoord /= 0.58; sprite split

	// 360x1024HD version still shows a glitch on the floor.
	vWindowCoord /= 0.55;

	//vWindowCoord /= 0.5;
	//vWindowCoord /= 0.45;
	
	//vWindowCoord *= .1 * 3600.0 / 60.0;

    // vec2 xy = -1.0 + 2.0*fragCoord.xy / iResolution.xy;
	// vec2 uv = xy * vec2(iResolution.x/iResolution.y,1.0);
	// Z:\jsc.svn\examples\javascript\chrome\apps\WebGL\synergy\m\ChromeShaderToyMountainsByHoskins\ChromeShaderToyMountainsByHoskins\Shaders\Program360.frag
    ...
    else
    {
        ...
        vCameraTarget = mix( vCameraTarget, vec3(1050, 30, -3500), smoothstep(28.0, 33.0, fCamTime));
		 ...
		 vCameraTarget = vCameraPos + vec3(0, 0, -116);

		// bottom
		if (uCameraTargetOffset.y == -1.0) 
		  vCameraTarget = vCameraPos + vec3(0, -116, -0.001);
    ...
    g_vRayDir = GetCameraRayDir( vWindowCoord, vCameraPos, vCameraTarget );
	  ...
	  mat3 camRotate = 
		...
		(uCameraTargetOffset.y == -1.0) ? 
			// rotationMatrix(vec3(0., 0., 1.), radians(0.)) 
			 rotationMatrix(vec3(0., 1., 0.), radians(90.0))
			:

		// top
		(uCameraTargetOffset.y == 1.0) ? 
			// rotationMatrix(vec3(1., 0., 0.), radians(0.)) 
			rotationMatrix(vec3(0., 1., 0.), radians(90.0))
		:
	...
	vec3 vForwards = normalize(g_vRayDir); 

    #ifdef ENABLE_SPRITES
	vec2 vSpriteDir = -normalize(vec2(-vForwards.z, vForwards.x));

	//vSpriteDir *= camRotate;
