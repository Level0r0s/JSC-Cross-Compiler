...
vec3 OrenNayarLightModel(vec3 rd, vec3 ld, vec3 n, float albedo)
{
	...
	float e0 = 4.8;
	col = vec3(.07,.4,.8) * vec3(albedo / mPi) * cos(aNLD) * (A + ( B * max(0.,cos(aRDN - aNLD)) * sin(alpha) * tan(beta)))*e0;
	return col;
}
...
vec2 map(vec3 p)
{
	...
	float left = p.x + pathLeft + 2. + voro - strateDisp - strateNoise;
	if (left < res.x)
		res = vec2(left, 2.);
	
	float right = 2. - p.x - pathRight + voro - strateDisp - strateNoise;
	if (right < res.x)
		res = vec2(right, 3.);
...
vec3 lighting(vec3 col, vec3 p, vec3 n, vec3 rd, vec3 ref, float t) // lighting    
{
	...
	vec3  lig = normalize( vec3(-0.2, 1., -0.2) );
...
void mainImage( inout vec4 f, in vec2 g )
{
    ...
    if (iMouse.z>0.) cv.y = iMouse.y/iResolution.y*.8 -.5;
	...
	if (d.x<DPrec.y)
	{
		...
		else if(s.y < 3.5)
		{
			...
			else if( s.y < 3.5) // right
			{
				f.rgb += strate(p.zy);	
			}		
		}
		
		f.rgb = lighting(f.rgb, p, n, rd, rd, s.x); // lighting    
   	}
	
	f = mix( f, vec4(vec3(.82,.63,.05), 1.), 1.0 - exp( -d.x*dstepf) ); 
}
