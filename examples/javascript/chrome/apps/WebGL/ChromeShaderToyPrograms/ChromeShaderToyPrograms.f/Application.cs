...
using ChromeShaderToyPrograms.f;
using ChromeShaderToyPrograms.f.Design;
using ChromeShaderToyPrograms.f.HTML.Pages;
...
namespace ChromeShaderToyPrograms.f
{
    ...
    public sealed class Application : ApplicationWebService
    {
        ...
        public Application(IApp page)
        {
            ...
            new IHTMLOption { value = "", innerText = $"{f.programs.Count} shaders available" }.AttachTo(combo);
            ...
            //Error CS0433  The type 'ShaderToy' exists in both 'ChromeShaderToyColumns, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null' and 'WebGL ShaderToy, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null'    ChromeShaderToyPrograms.f Z:\jsc.svn\examples\javascript\chrome\apps\WebGL\ChromeShaderToyPrograms\ChromeShaderToyPrograms.f\Application.cs   283
            ...
            f.programs.Keys.WithEachIndex(
                async (key, index) =>
                {
                    //var text = (1 + index) + " of " + References.programs.Count + " " + key.SkipUntilIfAny("ChromeShaderToy").Replace("By", " by ");
                    var text = (1 + index) + " of " + f.programs.Count + " " + key.SkipUntilIfAny("ChromeShaderToy").Replace("By", " by ");
