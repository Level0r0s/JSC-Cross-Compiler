...
using ChromeShaderToyPrograms.v;
using ChromeShaderToyPrograms.v.Design;
using ChromeShaderToyPrograms.v.HTML.Pages;
...
namespace ChromeShaderToyPrograms.v
{
    ...
    public sealed class Application : ApplicationWebService
    {
        ...
        public Application(IApp page)
        {
            ...
            //new IHTMLOption { value = "", innerText = $"{v.programs.Count} shaders available" }.AttachTo(combo);
            new IHTMLOption { value = "", innerText = v.programs.Count + "  shaders available" }.AttachTo(combo);
            ...
            //Error CS0433  The type 'ShaderToy' exists in both 'ChromeShaderToyColumns, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null' and 'WebGL ShaderToy, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null'    ChromeShaderToyPrograms.v Z:\jsc.svn\examples\javascript\chrome\apps\WebGL\ChromeShaderToyPrograms\ChromeShaderToyPrograms.v\Application.cs   283
            ...
            v.programs.Keys.WithEachIndex(
                async (key, index) =>
                {
                    //var text = (1 + index) + " of " + References.programs.Count + " " + key.SkipUntilIfAny("ChromeShaderToy").Replace("By", " by ");
                    var text = (1 + index) + " of " + v.programs.Count + " " + key.SkipUntilIfAny("ChromeShaderToy").Replace("By", " by ");
