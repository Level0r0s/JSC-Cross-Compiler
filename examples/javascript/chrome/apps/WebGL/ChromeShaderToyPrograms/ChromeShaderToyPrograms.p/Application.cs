...
using ChromeShaderToyPrograms.p;
using ChromeShaderToyPrograms.p.Design;
using ChromeShaderToyPrograms.p.HTML.Pages;
...
namespace ChromeShaderToyPrograms.p
{
    ...
    public sealed class Application : ApplicationWebService
    {
        ...
        public Application(IApp page)
        {
            ...
            new IHTMLOption { value = "", innerText = $"{p.programs.Count} shaders available" }.AttachTo(combo);
            ...
            //Error CS0433  The type 'ShaderToy' exists in both 'ChromeShaderToyColumns, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null' and 'WebGL ShaderToy, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null'    ChromeShaderToyPrograms.p Z:\jsc.svn\examples\javascript\chrome\apps\WebGL\ChromeShaderToyPrograms\ChromeShaderToyPrograms.p\Application.cs   283
            ...
            p.programs.Keys.WithEachIndex(
                async (key, index) =>
                {
                    //var text = (1 + index) + " of " + References.programs.Count + " " + key.SkipUntilIfAny("ChromeShaderToy").Replace("By", " by ");
                    var text = (1 + index) + " of " + p.programs.Count + " " + key.SkipUntilIfAny("ChromeShaderToy").Replace("By", " by ");
