...
using ChromeShaderToyPrograms.s;
using ChromeShaderToyPrograms.s.Design;
using ChromeShaderToyPrograms.s.HTML.Pages;
...
namespace ChromeShaderToyPrograms.s
{
    ...
    public sealed class Application : ApplicationWebService
    {
        ...
        public Application(IApp page)
        {
            ...
            new IHTMLOption { value = "", innerText = $"{s.programs.Count} shaders available" }.AttachTo(combo);
            ...
            //Error CS0433  The type 'ShaderToy' exists in both 'ChromeShaderToyColumns, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null' and 'WebGL ShaderToy, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null'    ChromeShaderToyPrograms.s Z:\jsc.svn\examples\javascript\chrome\apps\WebGL\ChromeShaderToyPrograms\ChromeShaderToyPrograms.s\Application.cs   283
            ...
            s.programs.Keys.WithEachIndex(
                async (key, index) =>
                {
                    //var text = (1 + index) + " of " + References.programs.Count + " " + key.SkipUntilIfAny("ChromeShaderToy").Replace("By", " by ");
                    var text = (1 + index) + " of " + s.programs.Count + " " + key.SkipUntilIfAny("ChromeShaderToy").Replace("By", " by ");
