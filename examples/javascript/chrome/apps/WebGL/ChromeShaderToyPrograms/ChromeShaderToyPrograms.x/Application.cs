...
using ChromeShaderToyPrograms.x;
using ChromeShaderToyPrograms.x.Design;
using ChromeShaderToyPrograms.x.HTML.Pages;
...
namespace ChromeShaderToyPrograms.x
{
    ...
    public sealed class Application : ApplicationWebService
    {
        ...
        public Application(IApp page)
        {
            ...
            new IHTMLOption { value = "", innerText = $"{x.programs.Count} shaders available" }.AttachTo(combo);
            ...
            //Error CS0433  The type 'ShaderToy' exists in both 'ChromeShaderToyColumns, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null' and 'WebGL ShaderToy, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null'    ChromeShaderToyPrograms.x Z:\jsc.svn\examples\javascript\chrome\apps\WebGL\ChromeShaderToyPrograms\ChromeShaderToyPrograms.x\Application.cs   283
            ...
            x.programs.Keys.WithEachIndex(
                async (key, index) =>
                {
                    //var text = (1 + index) + " of " + References.programs.Count + " " + key.SkipUntilIfAny("ChromeShaderToy").Replace("By", " by ");
                    var text = (1 + index) + " of " + x.programs.Count + " " + key.SkipUntilIfAny("ChromeShaderToy").Replace("By", " by ");
