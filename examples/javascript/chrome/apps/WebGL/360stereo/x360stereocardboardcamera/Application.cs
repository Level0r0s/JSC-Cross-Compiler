...
using x360stereocardboardcamera;
using x360stereocardboardcamera.Design;
using x360stereocardboardcamera.HTML.Pages;
using System.Diagnostics;
using ScriptCoreLib.JavaScript.WebGL;

namespace x360stereocardboardcamera
{
    using x360stereocardboardcamera.HTML.Images.FromAssets;
    ...
    public sealed class Application : ApplicationWebService
    {
        ...
        // subst /D b:
        // subst b: s:\jsc.svn\examples\javascript\chrome\apps\WebGL\x360stereocardboardcamera\x360stereocardboardcamera\bin\Debug\staging\x360stereocardboardcamera.Application\web
        // subst a: z:\jsc.svn\examples\javascript\chrome\apps\WebGL\x360stereocardboardcamera\x360stereocardboardcamera\bin\Debug\staging\x360stereocardboardcamera.Application\web
        // Z:\jsc.svn\examples\javascript\chrome\apps\WebGL\x360stereocardboardcamera\x360stereocardboardcamera\bin\Debug\staging\x360stereocardboardcamera.Application\web
        // what if we want to do subst in another winstat or session?

        // ColladaLoader: Empty or non-existing file (assets/x360stereocardboardcamera/S6Edge.dae)
        ...
        public Application(IApp page)
        {
            ...
            if (self_chrome_socket != null)
            {
                ...
                else
                {
                    ...
                    chrome.app.runtime.Launched += async delegate
                    {
                        ...
                        new chrome.Notification(title: "x360stereocardboardcamera");
            ...
            // THREE.WebGLRenderer: Texture is not power of two. Texture.minFilter is set to THREE.LinearFilter or THREE.NearestFilter. ( chrome-extension://aemlnmcokphbneegoefdckonejmknohh/assets/x360stereocardboardcamera/anvil___spherical_hdri_panorama_skybox_by_macsix_d6vv4hs.jpg )
            ...
            var frame2 = new IHTMLImage().AttachToDocument();
            ...
            var iskybox2 = new HTML.Images.FromAssets.IMG_20151205_150024_vr();
            var iskybox1 = new HTML.Images.FromAssets.IMG_20151205_150024_vr_jpg_GImageDataBytes();
            ...
            // 3840x2160
            var FullPanoWidthPixels = 8672;
            var FullPanoHeightPixels = 4336;

            var CroppedAreaTopPixels = 1346;
            var CroppedAreaImageHeightPixels = 1688;


            // https://sites.google.com/a/jsc-solutions.net/work/knowledge-base/15-dualvr/20151205/x360stereocardboardcamera
            var skybox2c = new CanvasRenderingContext2D(FullPanoWidthPixels, FullPanoHeightPixels);
            var skybox1c = new CanvasRenderingContext2D(FullPanoWidthPixels, FullPanoHeightPixels);


            // 1536
            // can we do a stereo cubemap?

            var stereocubemapheight = 1536;
            var stereocubemap = new CanvasRenderingContext2D(stereocubemapheight * 6 * 2, stereocubemapheight);
            ...
            Func<CanvasRenderingContext2D, Task> drawStereoFrame = async canvasTB =>
            {
                ...
                // https://sites.google.com/a/jsc-solutions.net/work/knowledge-base/15-dualvr/20151203/x360stereocardboardcamera

                // fake skybox?
                //canvasTB.fillStyle = "darkcyan";
                canvasTB.fillStyle = "black";
                canvasTB.fillRect(0, 0, canvasTB.canvas.width, canvasTB.canvas.height);
                ...
                canvasTB.drawImage(skybox1c.canvas, 0, 0, skybox1c.canvas.width, skybox1c.canvas.height, 0, canvasTB.canvas.height / 2, canvasTB.canvas.width, canvasTB.canvas.height / 2);
                ...
                new[] {
                    ...
                    (img, index) =>
                    {
                        stereocubemap.drawImage(
                            img.canvas,
                               stereocubemapheight * index,
                            // stereocubemapheight * (index + 6),
                            0,
                            stereocubemapheight,
                            stereocubemapheight
                        );
                ...
                canvasTB.drawImage(skybox2c.canvas, 0, 0, skybox1c.canvas.width, skybox1c.canvas.height, 0, 0, canvasTB.canvas.width, canvasTB.canvas.height / 2);
                //canvasTB.drawImage(gl.canvas, 0, 0, c.width, c.height, 0, 0, c.width, c.height / 2);
                //canvasTB.drawImage(gl.canvas, 0, 0, c.width, c.height, 0, c.height / 2, c.width, c.height / 2);
            ...
            new IHTMLButton { "make me a stero TB image " }.AttachToDocument().With(
                async e =>
                {
                    ...
                    while (await onclick)
                    {
                        ...
                        // cardboard camera needs the details to provide the crisp experience
            ...
            // https://sites.google.com/a/jsc-solutions.net/work/knowledge-base/15-dualvr/20151114/stereo
            // https://sites.google.com/a/jsc-solutions.net/work/knowledge-base/15-dualvr/20151112
            new IHTMLButton { "make me a stero cubemap image " }.AttachToDocument().With(
                async e =>
                {
                    // http://www.vrideo.com/watch/ALdE7mm
                    // https://www.youtube.com/watch?v=S3iTPxMIlCI
                    ...
                    while (await onclick)
                    {
                        ...
                        // 
                        //frame2.src = canvasTB8K.canvas.toDataURL();
                        frame2.src = stereocubemap.canvas.toDataURL();
            ...
            new { }.With(
                async delegate
                {
                    ...
                    await iskybox1.async.oncomplete;
                    skybox1c.drawImage(
                        iskybox1,
                        0,
                        CroppedAreaTopPixels,
                    ...
                    var skybox1_material = new THREE.MeshBasicMaterial(
                            new
                            {
                                map = new THREE.Texture { image = skybox1c, needsUpdate = true },
                    ...
                    var skybox2 = new THREE.Mesh(
                           ...
                           new THREE.MeshBasicMaterial(
                            new
                            {
                                map = new THREE.Texture { image = skybox2c, needsUpdate = true },
                                side = THREE.DoubleSide,
                                transparent = true
                            })
                    );

                    // http://stackoverflow.com/questions/29974578/how-to-flip-a-three-js-texture-horizontally
