...
namespace OVRWindWheelNDK
{
    ...
    {
        // ovr_AdjustClockLevels
        // https://sites.google.com/a/jsc-solutions.net/work/knowledge-base/15-dualvr/20150627

        public static string vrapi_GetVersionString() { return null; }

        // https://developer.oculus.com/licenses/mobile-3.2.1/
        // ? You can only distribute or re-distribute the source code to VRLib in whole, not in part.

        // Both LibOVR and VrAppFramework ship with full source. The VrApi is shipped as a set of public include files, 
        // a prebuilt shared library, and a jar file. Shipping VrApi as a separate shared library allows the VrApi
        // implementation to be updated and/or changed after an application has been released. This allows us to 
        // apply hot fixes, implement new optimizations, and add support for new devices without requiring 
        // applications to be recompiled with a new SDK. VrApi source is no longer included with the SDK.
    ...
    //   jni/OVRWindWheelNDK.dll.h:12:19: fatal error: VrApi.h: No such file or directory
    //#include "VrApi.h"
    //                  ^
    ...
    public unsafe struct ovrJava : VrApi_h
    {
        ...
        // Error	1	Cannot take the address of, get the size of, or declare a pointer to a managed type ('ScriptCoreLibNative.SystemHeaders.JavaVM')	X:\jsc.svn\examples\java\android\synergy\OVRVrCubeWorldSurfaceView\OVRWindWheelNDK\VrApi.cs	43	24	OVRWindWheelNDK
